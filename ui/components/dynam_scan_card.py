"""
Carte de contrôle pour le groupe Dynamo Scan LED (9 canaux).
Mapping : Ch1 Strobe, Ch2 Dimmer, Ch3 Pan (X), Ch4 Tilt (Y), Ch5 Vitesse,
Ch6 Couleur, Ch7 Gobo, Ch8 Mode, Ch9 Reset.
Layout aligné sur la carte Lyres : Frame Lumière, Couleurs, Gobos, Mouvement (pad XY, CENTER, 8, CIRCLE).
"""
...
# Taille 8 / Cercle : slider 0–1 <-> amplitude 0.05–0.5 (comme Lyres)
...
def _flood_style_color_button(button: ctk.CTkButton, color: str, active: bool) -> None:
    """Même rendu que les boutons couleur Lyres/FLOOD."""
    if active:
...
class DynamScanCard(ctk.CTkFrame):
    """
    Carte MOUVEMENTS pour Dynamo Scan LED (9 canaux).
    Structure comme la carte Lyres : Lumière, Couleurs, Gobos, Mouvement (pad XY, Vitesse Ch5, CENTER, 8, CIRCLE).
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        title = ctk.CTkLabel(
            self,
            text="DYNAMO SCAN LED (MOUVEMENTS)",
            anchor="w",
            font=("", 13, "bold"),
        )
        ...
        # ----- Frame Lumière (Ch2 Dimmer, Ch1 Strobe, OUT) -----
        ...
        # ----- Frame Couleurs : grille 2×5 uniquement (pas de preset U1–U4 pour Dynamo) -----
        ...
        # ----- Frame Gobos : grille 2 colonnes + Shake (même disposition que Lyres) -----
        ...
        # ----- Colonne Mouvement : Vitesse, Pad XY, CENTER, 8, CIRCLE (même disposition que Lyres) -----
        ...
        self.speed_slider = ctk.CTkSlider(
            speed_col, from_=0.0, to=1.0, orientation="vertical", number_of_steps=100,
            height=PAD_SIZE, width=18,
        ...
        self.amplitude_slider = ctk.CTkSlider(
            speed_col, from_=0.0, to=1.0, orientation="vertical", number_of_steps=100,
            height=PAD_SIZE, width=18,
            fg_color=DARK_CONSOLE_SECTION_BG, progress_color="#8b5cf6",
        ...
        # P1–P4 (gauche), CENTER (row 2), 8 / CIRCLE (col 2) — comme Lyres
    ...
    def set_active_position_index(self, index: Optional[int]) -> None:
        """Met en évidence le bouton de position rappelé (None = aucun)."""
    ...
    def update_position_labels(self, positions: List[Optional[Tuple[float, float]]]) -> None:
        """Met à jour le texte des boutons P1–P4 : P{pan} T{tilt} (0–255), ou P--/T-- si vide."""
        for idx, btn in enumerate(self._position_buttons):
            if idx < len(positions) and positions[idx] is not None:
                nx, ny = positions[idx][0], positions[idx][1]
    ...
    def _on_base_color(self, key: str, on_color_select: Callable[[str], None]) -> None:
        if self._active_color_key and self._active_color_key in self._color_buttons:
            prev = self._color_buttons[self._active_color_key]
            _flood_style_color_button(prev, getattr(prev, "_base_color", "#4b5563"), active=False)
        self._active_color_key = key
        btn = self._color_buttons.get(key)
        if btn:
            _flood_style_color_button(btn, getattr(btn, "_base_color", "#4b5563"), active=True)
        on_color_select(key)
    ...
    def _update_gobo_button_styles(self) -> None:
        for gid, btn in self._gobo_buttons.items():
            if gid == self._active_gobo_key:
                btn.configure(fg_color="#1e40af", hover_color="#2563eb", border_width=2, border_color="#0ea5e9")
            else:
                btn.configure(fg_color="#111827", hover_color="#1f2937", border_width=1, border_color="#4b5563")
