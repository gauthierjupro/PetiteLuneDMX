"""
Carte simplifiée pour le laser Ibiza LAS-30G (adresse 113).
OFF / AUTO / SOUND + Zoom (Ch5). Réf. Freestyler forum / doc utilisateur :
Tunnel 1 (Ch1) : 0-50 Sheltering (off), 51-101 Sound, 102-152 DMX mode 1,
153-203 DMX mode 2, 204-255 Auto.
"""
...
IbizaMode = Literal["off", "auto", "sound"]

# Valeurs DMX Canal 1 (Tunnel 1) d'après doc LAS-30G
IBIZA_MODE_OFF = 0     # 0-50 = Sheltering (off)
IBIZA_MODE_SOUND = 75  # 51-101 = Sound active
IBIZA_MODE_AUTO = 230  # 204-255 = Auto running


class IbizaCard(ctk.CTkFrame):
    """
    Carte simplifiée pour le laser Ibiza LAS-30G :
    OFF / AUTO / SOUND + Zoom / Taille (Ch5).
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._current_mode: IbizaMode = "off"
        ...
        title = ctk.CTkLabel(header, text="IBIZA LAS-30G (LASER)", anchor="w", font=("", 13, "bold"))
        ...
        self._off_btn.grid(row=0, column=0, padx=(0, 6))
        ...
        self._auto_btn.grid(row=0, column=1, padx=6)
        ...
        self._sound_btn.grid(row=0, column=2, padx=6)

        ctk.CTkLabel(content, text="Zoom / Taille (Ch5)", font=("", 10)).grid(row=2, column=0, sticky="w", pady=(0, 4))
        zoom_row = ctk.CTkFrame(content, fg_color="transparent")
        zoom_row.grid(row=3, column=0, columnspan=3, sticky="ew", pady=(0, 4))
    ...
    def _set_mode(self, mode: IbizaMode, callback: Callable[[str], None]) -> None:
    ...
    def set_mode(self, mode: IbizaMode) -> None:
        """Synchronise l'affichage du mode (sans déclencher le callback)."""
        self._current_mode = mode
        self._update_mode_buttons()

    def set_zoom_value(self, value: float) -> None:
        """Synchronise le slider zoom (0.0–1.0)."""
        self.zoom_slider.set(max(0.0, min(1.0, float(value))))


__all__ = ["IbizaCard", "IbizaMode", "IBIZA_MODE_OFF", "IBIZA_MODE_AUTO", "IBIZA_MODE_SOUND"]
